var myclass_prog=function(){
    ...
    this.loaddata=function(){
        ...
        setTimeout(() => {
            ...
            async=false;
            var res=this.callservice("logpfr.list",formData);
            if(res!=false){
                for (let i = 0; i < (res.datalist).length; i++) {
                    res.datalist[i].fecha_prf=(res.datalist[i].fecha_prf).split('/').reverse().join('/');
                }
    ...
    this.add=function(){
        ...
        arr_campos=[];
        arr_campos.push({ campo: 'fecha_prf', mensaje: 'Ingrese Fecha Válida.',tipo:'fecha' });
        ...
        */
        ...
        setTimeout(() => {
            ...
            _fecha=obtenerValor("fecha_prf");
            var _fechasave = _fecha.split('/').reverse().join('/');
            formData.set("fecha_prf",_fechasave);

            var _res=callservice("logpfr.insert",formData);
            console.log(_res);
            if(_res.ok="ok"){
    ...
    this.edit=function(id){
        ...
        for (let i = 0; i < (that.dataprog).length; i++) {
            ...
            if(_value.id==id){
                this.asignarValor("fecha_prf",_value.fecha_prf);
        ...
        swal({
            ...
            function (accion) {
                ...
                if (accion === true) {
                    ...
                    async=false;
                    var _res=callservice("logpfr.delete",formData);
                    if(_res.ok="ok"){
                        limpiarFormulario("_form");
                        that.loaddata();
                    }
